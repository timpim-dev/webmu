...
async function fetchCoverArt(rawName, ext) {
  const repo = COVER_REPOS[ext] || COVER_REPOS.md;
  const candidates = [
    rawName,
...
saveStateBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const saved = await WebMuGameSaves.saveStateToPB(gameName, 'genesis', state, 'Manual');
...
async function launchROM(rom, name) {
  ...
  try {
    ...
    const saveData = await WebMuGameSaves.loadStateFromPB(gameName, 'genesis');
    if (saveData) {
      try {
