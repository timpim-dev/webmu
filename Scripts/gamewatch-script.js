...
const COVER_REPOS = {
  mgw: 'Nintendo_-_Game_and_Watch',
};

function extOf(name) {
  const m = name.match(/\.(\w+)$/);
  return m ? m[1].toLowerCase() : 'mgw';
}
...
async function fetchCoverArt(rawName, ext) {
  const repo = COVER_REPOS[ext] || COVER_REPOS.mgw;
  const candidates = [
    rawName,
...
saveStateBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const saved = await WebMuGameSaves.saveStateToPB(gameName, 'gamewatch', state, 'Manual');
...
async function launchROM(rom, name) {
  ...
  try {
    ...
    const saveData = await WebMuGameSaves.loadStateFromPB(gameName, 'gamewatch');
    if (saveData) {
      try {
