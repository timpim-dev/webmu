...
async function fetchCoverArt(rawName, ext) {
  const repo = COVER_REPOS[ext] || COVER_REPOS.nes;
  const candidates = [
    rawName,
...
saveStateBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const saved = await WebMuGameSaves.saveStateToPB(gameName, 'snes', state, 'Manual');
...
async function launchROM(rom, name) {
  ...
  try {
    ...
    const saveData = await WebMuGameSaves.loadStateFromPB(gameName, 'snes');
    if (saveData) {
      try {
